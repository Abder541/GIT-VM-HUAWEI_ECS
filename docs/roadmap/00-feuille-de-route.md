...
- [~] Port de la base saine AWS : `crypto.ts`/`oidc.ts`/`sentry.ts`/`db.ts`/`email.ts` ✅ (typecheck OK). Reste : `presets.ts` (catalogue Huawei), `index.ts` (réconciliateur + routes), `web/`.
